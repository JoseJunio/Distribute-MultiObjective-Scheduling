	...
	public void execute(){
		...
		for(int i=0; i < this.maxIterations; i++){
			...
			if(this.crossoverOperator == 1) {
				...
				childs = Individual.crossover_two_point(parents, i+1, currentMaskepan, this.tasks);
			} else if(this.crossoverOperator == 3) {
				childs = Individual.crossover_uniform_unique(parents, i+1, currentMaskepan, this.tasks);
			} else if(this.crossoverOperator == 4) {
				childs = Individual.crossover_uniform_multiple(parents, i+1, currentMaskepan, this.tasks);
			}
			...
			if(this.mutationOperator == 1) {
				for(int j=0; j < childs.size(); j++) {
					...
					child.apply_mutation_simple(this.mutationFactor, currentMaskepan);
					makespanPopulation.insert_individual(child);
				}
			} else if(this.mutationOperator == 2) {
				for(int j=0; j < childs.size(); j++) {
					Individual child = childs.get(j);
					child.apply_mutation_uniform(this.mutationFactor, currentMaskepan);
					makespanPopulation.insert_individual(child);
				}
			}
